...
const MAX_DL_BYTES = 2 * 1024 * 1024 * 1024; // 2GB — WhatsApp doc limit
const PROGRESS_EVERY = 150 * 1024 * 1024;    // 150MB ට වරක් progress update
...
/** Stream download → disk (2GB safe, no RAM blowup) */
  ...
  return new Promise((resolve, reject) => {
    ...
    const go = (u) => {
      ...
      const req = lib.get(
        u,
        {
          timeout: 600000, // 10 min idle timeout
          headers: {
            'User-Agent': UA,
            Accept: '*/*',
        ...
        (res) => {
          ...
          // Content-Length තියෙනවා නම් 2GB limit check
          const cl = parseInt(res.headers['content-length'] || '0', 10);
          if (cl && cl > MAX_DL_BYTES) {
            res.resume();
            return reject(new Error(`File එක වඩාත් ලොකුයි (${formatBytes(cl)} > 2GB)`));
          }
          ...
          res.on('data', (c) => {
            ...
            if (received > MAX_DL_BYTES) {
              ...
              file.destroy();
              fs.unlink(filePath, () => {});
              reject(new Error(`File එක 2GB limit එක ඉක්මවා ගියා`));
              return;
            }
...
/** Flatten details.downloadLinks into a numbered flat list */
...
│  ›  ${prefix}movie <number>
│
│  ⬇️ *Download (document, up to 2GB)*
│  ›  ${prefix}movie dl <number>
│
│  ✨ Example:
│  ›  ${prefix}movie hotel transylvania
│  ›  ${prefix}movie 1
│  ›  ${prefix}movie dl 1
│
╰──────────────────────╯`,
    ...
    if (/^dl$/i.test(args[0]) && /^\d+$/.test(args[1] || '')) {
      ...
      if (!p || p.type !== 'links' || !p.links?.length) {
        return sock.sendMessage(
          from,
          { text: `❌ Active link list එකක් නැහැ.\n💡 \`${prefix}movie <name>\` වලින් පටන් ගන්න.` },
          { quoted: msg }
        );
      }
      ...
      const tmpPath = path.join(os.tmpdir(), `movie_${Date.now()}_${fname.replace(/[^\w.\-]/g, '_')}`);
      ...
      try {
        ...
        if (dl.bytes < 5000) throw new Error('File එක පොඩි වැරදියක් — retry කරන්න');

        // HTML/JSON error page check
        const fd = fs.openSync(tmpPath, 'r');
        const head = Buffer.alloc(40);
        fs.readSync(fd, head, 0, 40, 0);
        fs.closeSync(fd);
        const headStr = head.toString('utf8');
        if (headStr.includes('<html') || headStr.trim().startsWith('{')) {
          fs.unlinkSync(tmpPath);
          throw new Error('Server එකෙන් HTML ආවා — link expire වෙලා වෙන්න පුළුවන්');
        }

        await sock.sendMessage(
          from,
          { text: `📤 *WhatsApp එකට upload වෙමින්...*\n📦 ${formatBytes(dl.bytes)}\n⏳ මේකට වෙලා යනවා, ඉන්න...`, edit: loading.key },
        ).catch(() => {});

        const cap = `╭───「 🎬 *MOVIE HUB* 」───╮
│
│  📌 ${fname}
│  📦 *Size* › ${formatBytes(dl.bytes)}
│  🌐 ${target.host}
│
╰──────────────────────╯`;
        ...
        await sock.sendMessage(
          from,
          {
            document: { url: tmpPath },   // disk එකෙන් stream — RAM safe ✅
            mimetype: 'video/mp4',
            fileName: fname,
            caption: cap,
          },
        ...
        console.error('[MOVIE] download fail:', e.message);
        await sock.sendMessage(
          from,
          {
            text: `❌ Download fail — \`${e.message}\`\n\n🔗 Direct link එක:\n${target.url}\n\n💡 Browser/IDM එකෙන් download කරගන්න.`,
            edit: loading.key,
          }
        ).catch(() => {});
      } finally {
    ...
    if (args.length === 1 && /^\d+$/.test(args[0])) {
      ...
      if (idx < 0 || idx >= p.results.length) {
        return sock.sendMessage(
          from,
          { text: `❌ *1*–*${p.results.length}* තුළින් තෝරන්න.` },
          { quoted: msg }
        );
      }
      ...
      const loading = await sock.sendMessage(
        from,
        { text: `🎬 *${String(item.title).slice(0, 40)}*\n\n📖 Details ගන්නවා...` },
        { quoted: msg }
      );
      ...
      try {
        ...
        let body = `╭───「 🎬 *MOVIE DETAILS* 」───╮
│
│  📌 *Title* › ${String(d.title || item.title).slice(0, 55)}
│  🏷 *Site*  › ${hostOf(item.url)}
...
│`;
        ...
        if (d.description) {
          body += `│
│  📖 *Story*
│  ${String(d.description).replace(/\s+/g, ' ').slice(0, 140)}...
│`;
        }

        body += `│
│  ⬇️ *Download Links* (${links.length})
│
`;
        ...
        if (links.length > 12) body += `│  ...+${links.length - 12} more\n`;

        body += `│
│  💡 *${prefix}movie dl <number>* → document ⬇️ (2GB දක්වා)
│  ⏳ 5 min
╰──────────────────────╯`;
        ...
        if (thumb && /^https?:\/\//i.test(thumb)) {
          try {
            await sock.sendMessage(from, { image: { url: thumb }, caption: body }, { quoted: msg });
            return;
          } catch (_) {}
        }
        await sock.sendMessage(from, { text: body }, { quoted: msg });
      } catch (e) {
        console.error('[MOVIE] details fail:', e.message);
        await sock.sendMessage(
          from,
          { text: `❌ Details ගන්න බැරි වුණා.\n\`${e.message}\``, edit: loading.key }
        ).catch(() => {});
      }
      return;
    }
    ...
    const loading = await sock.sendMessage(
      from,
      { text: `🔍 *"${query.slice(0, 40)}"* හොයමින්... 🎬` },
      { quoted: msg }
    );

    try {
      const settled = await Promise.allSettled(
        SITES.map((s) => apiGet('/search', { site: s, q: query }))
      );
      ...
      let list = `╭───「 🎬 *MOVIE SEARCH* 」───╮
│
│  🔎 *${query.slice(0, 40)}*
│  📊 *${results.length}* results හමු වුණා ✨
│
`;
      results.forEach((r, i) => {
        list += `│  *${i + 1}.* ${String(r.title).slice(0, 46)}\n`;
        list += `│      🏷 ${hostOf(r.url)} · 📅 ${shortDate(r.date)}\n`;
      });
      list += `│
│  👇 *${prefix}movie <number>*
│  ⏳ 5 min
╰──────────────────────╯`;
      ...
      if (thumb && /^https?:\/\//i.test(thumb)) {
        try {
          await sock.sendMessage(from, { image: { url: thumb }, caption: list }, { quoted: msg });
          return;
        } catch (_) {}
      }
      await sock.sendMessage(from, { text: list }, { quoted: msg });
    } catch (e) {
      console.error('[MOVIE] search fail:', e.message);
      await sock.sendMessage(
        from,
        { text: `❌ Search fail.\n\`${e.message}\``, edit: loading.key }
      ).catch(() => {});
