...
async function saveIncomingMessage(sock, m) {
  try {
    ...
    // Download media for anti-delete resend
...
function attachFeatureHooks(sock, sessionId) {
  // Store messages for anti-delete + status seen
      ...
      for (const m of messages) {
        ...
        if (
          ...
          try {
            ...
            // Some Baileys versions also need this:
  ...
  sock.ev.on('messages.update', async (updates) => {
    try {
      ...
      for (const u of updates) {
        ...
        // Some builds: message set null
        if (u.update && u.update.message === null) isRevoke = true;
        // Stub types for revoke
        ...
        // Original message id is inside protocolMessage.key
