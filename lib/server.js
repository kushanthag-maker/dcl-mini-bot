...
function startServer() {
  ...
  app.get('/api/status', (req, res) => {
    const status = typeof global.getBotStatus === 'function' ? global.getBotStatus() : { status: 'starting' };
    res.json(status);
  });
  ...
  app.listen(config.port, config.host, () => {
    logger.success(`Pairing website running at http://localhost:${config.port}`);
  });
