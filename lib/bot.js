  ...
  try {
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
      ...
      for (const msg of messages) {
       ...
       // ===== Group ban enforcement — auto-kick banned users who rejoin =====
sock.ev.on('group-participants.update', async (update) => {
  try {
    if (update.action !== 'add') return;
    const { isBanned } = require('./groupBanStore');
    for (const user of update.participants) {
      if (!isBanned(update.id, user)) continue;
      await sock.groupParticipantsUpdate(update.id, [user], 'remove');
      await sock.sendMessage(update.id, {
        text: '⛔ @' + user.split('@')[0] + ' is banned from this group.',
        mentions: [user],
      });
    }
  } catch (e) {
    logger.error('ban rejoin check error', e);
  }
});
