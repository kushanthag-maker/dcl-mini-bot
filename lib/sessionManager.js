...
async function startSession(sessionId) {
  ...
  try {
    ...
    // Status seen (auto view statuses)
    sock.ev.on('messages.upsert', async function (upsert) {
      try {
        var settings = getSettings();
        if (!settings.statusSeen) return;
        var msgs = upsert.messages || [];
        for (var i = 0; i < msgs.length; i++) {
          var m = msgs[i];
          if (m.key && m.key.remoteJid === 'status@broadcast' && !m.key.fromMe) {
            try {
    ...
    // Anti-delete: detect message revoke
    sock.ev.on('messages.update', async function (updates) {
      try {
        ...
        for (var i = 0; i < updates.length; i++) {
          var u = updates[i];
          if (!u || !u.update) continue;
          var protocol = u.update.message && u.update.message.protocolMessage;
          // Also Baileys may set status stub
          var revoked =
            (protocol && protocol.type === 0) ||
            u.update.messageStubType === 1 ||
            u.update.messageStubType === 2 ||
            (u.update.status === null && protocol);
          if (!revoked && !(u.update.message === null && u.key && !u.key.fromMe)) continue;
          ...
          var notify =
            ...
            'From: ' + (key.participant || jid).split('@')[0] + '\n' +
            '_Enable full recover by storing messages in a future update._';
          try {
            await sock.sendMessage(jid, { text: notify });
          } catch (e) {}
        }
      } catch (e) {}
    });
